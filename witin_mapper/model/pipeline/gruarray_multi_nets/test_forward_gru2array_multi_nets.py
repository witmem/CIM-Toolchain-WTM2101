...
np.random.seed(0)
root_dir = "./tests/onnx/witin/wtm2101/customer/gruarray_multi_nets/"
...
def get_data_vp3(root_dir):
    data = np.load(root_dir + + '/layer_debug/gru1_in.npy').astype('float32')
    return data
...
def test_forward_gru2array_dnn_multi_net():
    ...
    new_data_3 = np.round(np.random.rand(111, 224) * 255).astype("float32") - 128
    ...
    m.set_input(2, witin.nd.array(new_data_3))
    m.run()
