...
def generate_model():
    ...
    output2_shape = [1, 3, 16, 16]
    kernel2_tensor_shape = [3, 1, 2, 2]
    ...
    reduceMean_node = onnx.helper.make_node("ReduceMean", inputs=["conv1_relu_out"], axes=[1], keepdims=True, outputs=["reduceMean_out"])
    ...
    conv2_node = onnx.helper.make_node("Conv",
                                       inputs=['reduceMean_out', 'conv2_w_tensor', 'conv2_b_tensor'],                                   
                                       outputs=['conv2'],                                        
    ...
    nodes = node1 + [reduceMean_node] + node2  
    initializers = initializer1 + initializer2   
    graph = onnx.helper.make_graph(nodes, "cnn_reduceMean_net",
    ...
    model = onnx.helper.make_model(graph, producer_name='cnn_reduceMean_net')  
    with open(file_dir + "cnn_reduceMean_net.onnx", "wb") as f:    
        f.write(model.SerializeToString())      
    print("Generate cnn_reduceMean_net sucessfully!")

def run_model():     
    input_shape = [1, 8, 64, 64]    
    cnn_model = onnx.load(file_dir + "cnn_reduceMean_net.onnx")  
