...
def generate_model():
    ...
    conv2_node = onnx.helper.make_node("Conv",
                                       inputs=['softmax_out', 'conv2_w_tensor', 'conv2_b_tensor'],
                                       outputs=['conv2'],
    ...
    nodes = conv1_nodes + [softmax_node] + conv2_nodes
    ...
    with open(file_dir + "softmax.onnx", "wb") as f:
        f.write(dccrn_model.SerializeToString())
    print("Generate softmax.onnx sucessfully!")
	

def run_model():
    input_shape = [1, 8, 64, 64]
    params = []
    conv_model = onnx.load(file_dir + "softmax.onnx")
