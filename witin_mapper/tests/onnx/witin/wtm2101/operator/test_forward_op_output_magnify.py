...
def npu_graph_build(mods, params, inputs, optimize_config):
    ...
    optimize_path = "./model/optimize_layer/op_output_magnify.protobuf"
    ...
    with witin.transform.PassContext(opt_level=3):
        _, _, _, npu_graph = witin_frontend.build_module.build(
            ...
            optimize_method_config=optimize_path,
...
def get_tvm_output(model_path):
    ...
    runtime_model = npu_graph_build(mods, params, inputs, None)
    runtime_model.run()
...
def generate_tdnn_model():
    ...
    date_time = datetime.datetime.now()
    timestamp = date_time.strftime("%d_%b_%Y_%H_%M_%S_%f")
    model_path = "./model/optimizer_tdnn_dnn_" + timestamp + ".onnx"
