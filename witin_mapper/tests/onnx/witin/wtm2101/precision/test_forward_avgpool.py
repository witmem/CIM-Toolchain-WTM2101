...
onnx_dir = './model/pipeline/average_pool_model/'
...
def run_model():
    ...
    params = []
    input_datas = []
    avg_pool_model = onnx.load(onnx_dir + "avgpool_runtime_test.onnx")
    

    shape_dict_avg_pool = {}
    shape_dict_avg_pool['in'] = (1, 4, 64, 64)  #NCHW

    mod_avg_pool, param_avg_pool = witin_frontend.frontend.from_onnx(avg_pool_model, shape_dict_avg_pool)
    data_avg_pool =  np.random.randint(0,127,size=(100, 4,64, 64), dtype='int8').astype('float32')
