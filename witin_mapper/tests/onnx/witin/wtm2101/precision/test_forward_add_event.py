...
def generate_model():
    ...
    add_node = onnx.helper.make_node('Add', ['conv1_relu', 'conv2_relu'], ['out'],  shift_bit=-1)
    
    
    nodes = node1 + node2 + [add_node]
    initializers =  initializer1 + initializer2

    input1_shape = (1, 4, 50, 50)
    input2_shape = (1, 4, 50, 50)
    output_shape = (1, 4, 50, 50)
    ...
    dccrn_model = onnx.helper.make_model(dccrn_graph, producer_name='add_event_model')

    with open(onnx_dir + '/add_event_model.onnx', "wb") as f:
        f.write(dccrn_model.SerializeToString())
    print("Generate add_event_model sucessfully!")


def test_multiple_input_add_model():
    generate_model()
    mods = []
    params = []
    input_datas = []

    add_model = onnx.load(onnx_dir + "add_event_model.onnx")
    
    shape_dict_add = {}
    shape_dict_add['in1'] = (1, 4,50, 50)  #NCHW
    shape_dict_add['in2'] = (1, 4,50, 50)  #NCHW

    mod_add, param_add = witin_frontend.frontend.from_onnx(add_model, shape_dict_add)
    mods.append(mod_add)
    params.append(param_add)
    
    data1 =  np.random.randint(0,127,size=(100, 50, 50, 4), dtype='int8').astype('float32')
    data2 =  np.random.randint(0,127,size=(100, 50, 50, 4), dtype='int8').astype('float32')
