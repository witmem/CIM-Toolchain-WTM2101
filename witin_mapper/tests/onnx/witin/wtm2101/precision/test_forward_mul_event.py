...
def generate_model():
    ...
    # uint8 + uint8 => uint8, need right shift 8 bits
    mul_node = onnx.helper.make_node('Mul', ['conv1_relu', 'conv2_relu'], ['out'], shift_bit=-8)
    
    nodes = node1 + node2 + [mul_node]
    initializers =  initializer1 + initializer2

    input1_shape = (1, 4, 32, 64)
    input2_shape = (1, 4, 32, 64)
    output_shape = (1, 4, 32, 64)
    ...
    dccrn_model = onnx.helper.make_model(dccrn_graph, producer_name='mul_event_model')

    with open(onnx_dir + '/mul_event_model.onnx', "wb") as f:
        f.write(dccrn_model.SerializeToString())
    print("Generate mul_event_model sucessfully!")
...
def run_model():
    ...
    add_model = onnx.load(onnx_dir + "mul_event_model.onnx")
    
    shape_dict_add = {}
    shape_dict_add['in1'] = (1, 4,32, 64)  #NCHW
    shape_dict_add['in2'] = (1, 4,32, 64)  #NCHW

    mod_add, param_add = witin_frontend.frontend.from_onnx(add_model, shape_dict_add)
    mods.append(mod_add)
    params.append(param_add)
    
    data_add1 =  np.random.randint(0,127,size=(100, 64, 32, 4), dtype='int8').astype('float32')
    data_add2 =  np.random.randint(0,127,size=(100, 64, 32, 4), dtype='int8').astype('float32')
    ...
    m.set_input(1, witin.nd.array(data_add1))
