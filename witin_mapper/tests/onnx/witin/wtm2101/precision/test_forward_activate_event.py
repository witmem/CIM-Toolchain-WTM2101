...
def generate_model():
    ...
    concat_node = onnx.helper.make_node('Concat', ['conv1_relu', 'conv2_relu'], ['concat_out'],  axis = 1)
    # concat_scale_node = onnx.helper.make_node("Scale",
    #                                               inputs=['concat_out'],
    #                                               outputs=['out'],
    #                                               scale=1.0 / 2)

    concat_relu_node = onnx.helper.make_node("Relu",['concat_out'], outputs=['out'])
    #                                              scale=1.0 / 2)
    
    
    nodes = node1 + node2 + [concat_node] + [concat_relu_node]
    initializers =  initializer1 + initializer2

    input1_shape = (1, 6, 16, 16)
    input2_shape = (1, 6, 16, 16)
    ...
    dccrn_model = onnx.helper.make_model(dccrn_graph, producer_name='acti_event_model')

    with open(onnx_dir + '/acti_event_model.onnx', "wb") as f:
        f.write(dccrn_model.SerializeToString())
    print("Generate acti_event_model sucessfully!")


def test_multiple_input_activate_model():
    generate_model()
    mods = []
    params = []
    input_datas = []
    ...
    add_model = onnx.load(onnx_dir + "acti_event_model.onnx")
    

    shape_dict_add = {}
    shape_dict_add['in1'] = (1, 6,16, 16)  #NCHW
    shape_dict_add['in2'] = (1, 6,16, 16)  #NCHW

    mod_add, param_add = witin_frontend.frontend.from_onnx(add_model, shape_dict_add)
    mods.append(mod_add)
    params.append(param_add)
    
    data1 =  np.random.randint(0,127,size=(100, 16, 16, 6), dtype='int8').astype('float32')
    data2 =  np.random.randint(0,127,size=(100, 16, 16, 6), dtype='int8').astype('float32')
...
if __name__ == "__main__":
    test_multiple_input_activate_model()
