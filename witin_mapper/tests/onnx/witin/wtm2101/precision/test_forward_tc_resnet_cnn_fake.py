...
from engine.npu_build import build_run_model

np.random.seed(0)

onnx_dir = './model/pipeline/cnn_tc_resnet_model/'
...
def test_huawei_cnn_new_net_case_new_add_scale():
    ...
    # onnx_model = infer_shapes(conv_model_left)
    ...
    mods.append(mod_conv_left)
    params.append(param_conv_left)
    input_datas.append(input_dt_conv_left)
    ...
    data_conv_right = np.clip(data_conv_right,0,255)
    ...
    mods.append(mod_conv_right)
    params.append(param_conv_right)
    input_datas.append(input_dt_conv_right)
    ...
    mods.append(mod_gemm)
    params.append(param_gemm)
    input_datas.append(input_dt)

    opt_config = './model/pipeline/cnn_tc_resnet_model/hua_dw_tc_opti.protobuf'
    dateTimeObj = datetime.datetime.now()
    timestampStr = dateTimeObj.strftime("%d_%b_%Y_%H_%M_%S_%f")
    build_dir = "./output/output_" + os.path.basename(__file__).split(".")[0] + timestampStr

    build_run_model(mods, params, input_datas, opt_config, build_dir)
