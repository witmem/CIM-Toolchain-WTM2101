...
def _convert_dense(inexpr, keras_layer, etab):
    ...
    if input_dim == 3:
        ...
        for index, item in enumerate(input_shape):
            if index > 0:
                new_input_shape.append(item)

        new_input_dim = len(new_input_shape)
    else:
    ...
    if new_input_dim > 2:
        new_input_shape = tuple(dim if dim else 1 for dim in _as_list(new_input_shape)[0])
        if new_input_dim != 3 or new_input_shape[0] != 1 or new_input_shape[1] != 1:
            raise tvm.error.OpAttributeInvalid(
                'Input shape {} is not valid for operator Dense.'.format(new_input_shape))
...
def from_keras(model, shape=None, layout='NCHW'):
    ...
    def _convert_input_layer(keras_layer):
        ...
        etab.set_expr(input_name, new_var(input_name, shape=new_input_shape))
    ...
    for keras_layer in model.layers:
        if isinstance(keras_layer, input_layer_class):
            ...
            for node_idx, node in enumerate(inbound_nodes):
                ...
                zip_node = zip(
                    _as_list(0),
                    #_as_list(node.node_indices), #afly backup
                    _as_list(0),
                    #_as_list(node.tensor_indices)) #afly backup
