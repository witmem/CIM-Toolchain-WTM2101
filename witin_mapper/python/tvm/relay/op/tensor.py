...
def npu_add(lhs,rhs,shift_bit):
    return _make.npu_add(lhs,rhs,shift_bit)
...
def npu_mul(lhs,rhs,shift_bit):
    return _make.npu_mul(lhs,rhs,shift_bit)

def scale(data, scale):
    return _make.scale(data,scale)

def floatScale(data, scale, clip_bit):
    return _make.floatScale(data, scale, clip_bit)

def ReduceMean(data, axis, keepdims=False):
    axis = [axis] if isinstance(axis, int) else axis
    return _make.ReduceMean(data, axis, keepdims)

def scalar(data,scale,shift_bit,op_types):
    return _make.scalar(data,scale,shift_bit,op_types)

def clearLowBits(data, bits, node_name):
    return _make.clearLowBits(data, bits, node_name)
