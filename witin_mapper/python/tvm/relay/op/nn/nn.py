...
def conv1d(data,
    ...
    return _make.conv1d(data, weight, strides, padding, dilation,
                        ...
                        kernel_layout, out_layout, out_dtype, node_name)
...
def conv2d_transpose(data,
    ...
    return _make.conv2d_transpose(data, weight, strides, padding, dilation,
                                  groups, channels, kernel_size, data_layout,
                                  kernel_layout, out_layout, output_padding,
                                  out_dtype, node_name)
...
def softmax(data, axis=-1, scale_in = 1.0, scale_out = 1.0):
    r"""Computes softmax.
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.softmax(data, axis, scale_in, scale_out)

def sumnorm(data, axis=-1, scale_in = 1.0, scale_out = 1.0):
    r"""Computes sumnorm.

    .. math:: \text{sumnorm}(x)_i = \frac{exp(x_i)}{\sum_j exp(x_j)}
    ...
    axis: int, optional
        The axis to sum over when computing sumnorm
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.sumnorm(data, axis, scale_in, scale_out)
...
def avg_pool2d(data,
    ...
    return _make.avg_pool2d(data, pool_size, strides, padding,
                            layout, ceil_mode, count_include_pad, scale_in, scale_out)
...
def global_avg_pool2d(data,
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.global_avg_pool2d(data, layout, scale_in, scale_out)
...
def dense(data, weight, units=None, out_dtype="", node_name = ""):
    """Dense operator.
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.dense(data, weight, units, out_dtype, node_name)

def matmul(data, weight, node_name = ""):
    """Dense operator.
    Applies matrix multiplication

    .. math::

    `Y[dim1, dim3] = X[dim1, dim2] * W[dim2, dim3]
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.matmul(data, weight, node_name)

def tdnn(data, weight, time_offsets, bias_params, scale, out_dtype="", node_name = ""):
    """Tdnn operator.
    Applies a linear transformation
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.tdnn(data, weight, time_offsets, bias_params, scale, out_dtype, node_name)


def act_lut(data, table, act_type, node_name):
    return _make.act_lut(data, table, act_type, node_name)
...
def lstm(data,
         ...
         out_dtype="",
         node_name=""):
    return _make.lstm(data, weight1, weight2, bias1, bias2, act_types, act_tables, scale_ioft,
                      scale_ct, shift_bits, clean_ctht, out_dtype, node_name)
...
def gru(data,
        ...
        node_name=""):
    if weight3 == None:
      return _make.gru(data, weight1, weight2, bias1, bias2, act_types, act_tables, scale_zr,
                       scale_ht, scale_ones, shift_bits, clean_ht, out_dtype, node_name)
    else:
      return _make.gruNew(data, weight1, weight2, weight3, bias1, bias2, bias3, act_types, act_tables, scale_zr,
                          scale_ht, scale_in, scale_ones, shift_bits, clean_ht, out_dtype, node_name)
...
def gru2array(data,
        ...
        out_dtype="",
        node_name=""):
    return _make.gru2Array(data, weight1, weight2, bias1, bias2, act_types, act_tables, input_offset,hidden_shift_bit, 
                           scale_input,scale_hidden, scale_ones, shift_bits, clean_ht, out_dtype, node_name)
    

def splice(data, axis=None, delay=None, out_dtype=""):
    """splice operator.
    Applies a linear transformation
    ...
    result : tvm.relay.Expr
        The computed result.
    """
    return _make.splice(data, axis, delay, out_dtype)
...
def batch_norm_customed(data,
                        ...
                        scale_in=1.0,
                        scale_out=1.0):
    return _make.batch_norm_customed(data, gamma, beta, mean, var, epsilon, scale_in, scale_out)
...
def group_norm(data,
    ...
    result : tvm.relay.Expr
        The normalized data.
    """
    return _make.group_norm(data, gamma, beta, num_groups, num_channels, scale_out, node_name, axis, epsilon, center, scale)
